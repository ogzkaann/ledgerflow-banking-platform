...
class KafkaFailureConfiguration {
    @Bean
    DefaultErrorHandler transferKafkaErrorHandler(KafkaTemplate<String, String> kafka, MeterRegistry meters) {
        var recoverer = new DeadLetterPublishingRecoverer(kafka, (record, exception) -> {
            meters.counter("kafka.dlt.publications", "service", "transfer-service")
                    .increment();
        ...
        handler.setRetryListeners((record, exception, deliveryAttempt) -> meters.counter(
                        "kafka.listener.failures", "service", "transfer-service")
                .increment());
