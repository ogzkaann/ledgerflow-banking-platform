...
public class TransferSecurityConfiguration {
    ...
    @Bean
    SecurityFilterChain transferSecurityFilterChain(HttpSecurity http, MeterRegistry meters) throws Exception {
        return http.sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
                ...
                .oauth2ResourceServer(oauth -> oauth.jwt(jwt -> jwt.jwtAuthenticationConverter(realmRoles()))
                        .authenticationEntryPoint((request, response, exception) -> {
                            meters.counter("security.authentication.failures", "service", "transfer-service")
                                    .increment();
                            problem(response, 401, "Unauthorized");
                        })
                        .accessDeniedHandler((request, response, exception) -> {
                            meters.counter("security.authorization.denials", "service", "transfer-service")
                                    .increment();
