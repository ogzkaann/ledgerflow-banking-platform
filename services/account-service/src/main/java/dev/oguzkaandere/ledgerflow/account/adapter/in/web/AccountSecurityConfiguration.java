...
public class AccountSecurityConfiguration {
    ...
    @Bean
    SecurityFilterChain accountSecurityFilterChain(HttpSecurity http, MeterRegistry meters) throws Exception {
        return http.sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
                ...
                .authorizeHttpRequests(authorize -> authorize
                        ...
                        .requestMatchers(HttpMethod.POST, "/api/v1/accounts/*/test-funding")
                        .hasRole("ledgerflow-admin")
                ...
                .oauth2ResourceServer(oauth -> oauth.jwt(jwt -> jwt.jwtAuthenticationConverter(realmRoles()))
                        .authenticationEntryPoint((request, response, exception) -> {
                            meters.counter("security.authentication.failures", "service", "account-service")
                                    .increment();
                            problem(response, 401, "Unauthorized");
                        })
                        .accessDeniedHandler((request, response, exception) -> {
                            meters.counter("security.authorization.denials", "service", "account-service")
                                    .increment();
