...
class KafkaFailureConfiguration {
    @Bean
    DefaultErrorHandler accountKafkaErrorHandler(KafkaTemplate<String, String> kafka, MeterRegistry meters) {
        var recoverer = new DeadLetterPublishingRecoverer(kafka, (record, exception) -> {
            meters.counter("kafka.dlt.publications", "service", "account-service")
                    .increment();
        ...
        handler.setRetryListeners((record, exception, deliveryAttempt) -> meters.counter(
                        "kafka.listener.failures", "service", "account-service")
                .increment());
