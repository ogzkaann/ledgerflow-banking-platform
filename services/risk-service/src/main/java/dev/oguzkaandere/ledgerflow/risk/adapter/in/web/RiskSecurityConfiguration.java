package dev.oguzkaandere.ledgerflow.risk.adapter.in.web;
...
public class RiskSecurityConfiguration {

    @Bean
    SecurityFilterChain riskSecurityFilterChain(HttpSecurity http, MeterRegistry meters) throws Exception {
        return http.sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
                ...
                .oauth2ResourceServer(oauth -> oauth.jwt(jwt -> jwt.jwtAuthenticationConverter(realmRoles()))
                        .authenticationEntryPoint((request, response, exception) -> {
                            meters.counter("security.authentication.failures", "service", "risk-service")
                                    .increment();
                            problem(response, 401, "Unauthorized");
                        })
                        .accessDeniedHandler((request, response, exception) -> {
                            meters.counter("security.authorization.denials", "service", "risk-service")
                                    .increment();
